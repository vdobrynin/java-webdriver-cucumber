...
public class UspsStepDefs {
  ...
  public void iFillOutStreetCityState(String street, String city, String state) {
    ...
    //    getDriver().findElement(By.xpath("//select[@id='tState']/option[@value='" + state + "']")).click(); // passing 'state'
    WebElement dropdown = getDriver().findElement(By.xpath("//select[@id='tState']"));            // optimization from line above
  ...
  public void iValidateZipCodeExistsInTheResult(String zip) throws InterruptedException {
    //    Thread.sleep(5000);                                                                             // do not need any more lecture #8
    ...
    wait.until(ExpectedConditions.textToBePresentInElementLocated(By.xpath("//div[@id='zipByAddressDiv']"), zip));  //--> it assertion itself
  ...
  public void iSelectedWithShape(String str1, String str2) {
    getDriver().findElement(By.xpath("//select[@id='CountryID']//option[contains(text(),'United Kingdom (United Kingdom of Great Britain an')]")).click();
    getDriver().findElement(By.xpath("//input[@id='option_1']")).click(); //div[@id='options-section']//input[@id='option_1']
  }
  ...
  public void iCalculateThePriceAndValidateCostIs(String price) {
    String priceToVerify = getDriver().findElement(By.xpath("//div[@id='wrap']//div[@class='container']")).getText();
  ...
  public void iGoToTab(String tab) {
    ...
    getDriver().findElement(By.xpath("//a[text()='" + tab + "']")).click();  // it's easy to access through "tab" everywhere
  }
  ...
  public void iSearchAndValidateNoProductsFound() {
    ...
    WebElement noResults = getDriver().findElement(By.xpath("//div[@class='no-results-found']"));
  ...
  public void chooseCategoryPriorityMail() {
    getDriver().findElement(By.xpath("//label[contains(@for, 'Service-Priority Mail-')]")).click();
  }
  ...
  public void iUnselectStampsCheckbox() {
    getDriver().findElement(By.xpath("//label[@for='checkbox-type-Category-Stamps'][contains(text(),'Stamps (92)')]")).click();
  }

  @And("select size {string}")
  public void selectSize(String arg0) {
    getDriver().findElement(By.xpath("//label[@for='checkbox-type-Size-Large-11'][contains(text(), 'Large')]")).click();
  }
  ...
  public void iVerifyAndFilters(String arg0, String arg1) {
    WebElement result = getDriver().findElement(By.xpath("//div[@class='breadcrumb-cartridge']"));
  ...
  public void iVerifySale(String arg0) {
    WebElement result = getDriver().findElement(By.xpath("//div[@class='result-products-holder']"));
    assertThat(result.isDisplayed()).isTrue();
  ...
  public void iPerformSearch(String text) throws InterruptedException {
    ...
    new Actions(getDriver()).moveToElement(search).click(searchInput).sendKeys(text).sendKeys(Keys.ENTER).perform();
    //    Thread.sleep(2000);
  }
  ...
  public void iSelectInResults(String resultText) {     //span[contains(text(),'Passport Application')]
    ...
    getExecutor().executeScript("arguments[0].click();", getDriver().findElement(By.xpath("//span[contains(text(),'" + resultText + "')]")));
  }

  @And("I click {string} button")
  public void iClickButton(String buttonText) {
    //a[@class='button--primary'][contains(text(),'Find a Post Office')]
    getDriver().findElement(By.xpath("//a[@class='button--primary'][contains(text(),'" + buttonText + "')]")).click();
  }

  @And("I select {string} zip code within {string} and search")
  public void iSelectZipCodeWithinAndSearch(String zip, String miles) {
    getDriver().findElement(By.xpath("//input[@id='tCityStateZip']")).sendKeys(zip); // input zip
    getDriver().findElement(By.xpath("//div[@id='sWithinList']")).click();
    //div[@id='sWithinList']//a[contains(text(),'10 miles')]
    getDriver().findElement(By.xpath("//div[@id='sWithinList']//a[text()= '" + miles + "']")).click(); // choose dropDown for miles
  ...
  public void iVerifyPresentInSearchResults(String city) {  //div[@id='search-results']
    WebElement search = getDriver().findElement(By.xpath("//div[@id='polo-search-form']//input[@id='bSearch']"));
    getExecutor().executeScript("arguments[0].click();", search);
    WebElement result = getDriver().findElement(By.xpath("//div[@class='polocator-info']"));
    assertThat(result.isDisplayed()).isTrue();
  ...
  public void iGoToUnder(String menuItem, String menu) { //a[contains(text(),'Track & Manage')]
    ...
    WebElement menuElement = getDriver().findElement(By.xpath("//a[contains(text(),'" + menu + "')]"));
    new Actions(getDriver()).moveToElement(menuElement).perform();                                        // --> mouse over
    getDriver().findElement(By.xpath("//a[contains(text(),'" + menuItem + "')]")).click(); //a[contains(text(),'PO Boxes')]
  }
  ...
  public void iVerifyThatPresent(String location) {
    ...
    String actualSearchResult = getDriver().findElement(By.xpath("//div[@id='locationContainer_1']")).getText(); // --> Vlad's
...
//        assertThat(actualSearchResult.getText().compareToIgnoreCase(location));
  ...
  public void iVerifyThatPOBoxIsAvailableIn(String size, String location) {
                              ...
                              //h2[text()='Los Altos — Post Office™'] --> change 'text' to 'string'( string will allow all combination of the text)
    getDriver().findElement(By.xpath("//h2[string()='" + location + "']")).click(); //h2[string()='Los Altos — Post Office™'] --> change to +location+
